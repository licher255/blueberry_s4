...
> ⚠️ **警告：驱动安装是启动项目的前提**
>
> 在启动项目之前，**必须确保 `drivers/` 目录下的内核驱动已正确编译、安装并加载**。这是整个项目正常运行的前提条件，否则 CAN 设备将无法识别，导致硬件通信失败。
>
> 验证方法：`lsmod | grep -E "pcan|usbcanfd"` 应显示驱动已加载
>
> 详见下方的 [1.2 编译 CAN 驱动](#12-编译-can-驱动如需要) 章节
...
> ⚠️ **警告：执行此步骤前，请确保已安装驱动！**
>
> 如果 `lsmod | grep -E "pcan|usbcanfd"` 没有输出，请先完成 [1.2 编译 CAN 驱动](#12-编译-can-驱动如需要)。
