...
│                  CAN 总线分离架构                   │
├──────────────────────────┬─────────────────────────┤
│     PEAK PCAN (500K)     │   ZLG CAN-FD (1M/5M)    │
│  标准 CAN + 扩展帧混合    │        CAN-FD           │
├──────────────────────────┼─────────────────────────┤
│  AGV (扩展帧 0x18C4xxxx)  │                         │
│  Kinco (标准帧 0x181)     │    WHJ (CAN-FD 0x07)    │
└──────────────────────────┴─────────────────────────┘
```

**设计决策**:
- **AGV + Kinco** 共用 PEAK PCAN @ 500K（扩展帧和标准帧可共存）
- **WHJ** 独立 ZLG CAN-FD @ 1M/5M（必须使用 CAN-FD）
- **分离原因**: WHJ 需要 1Mbps，AGV/Kinco 只能 500K，波特率不兼容

**注意事项**:
- AGV 使用 29-bit 扩展帧，Kinco 使用 11-bit 标准帧，ID 空间不冲突
- Kinco CANopen ID: 0x181 (TPDO1), 0x281 (TPDO2), 遵循 0x180+NodeID 规范
- WHJ Node ID: 7，独立 CAN-FD 总线
